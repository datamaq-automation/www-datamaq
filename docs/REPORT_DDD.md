...
3. **Chatwoot hoy:** solo existe integración de widget vía SDK en `ChatwootManager.js`. No hay creación de Contact/Conversation desde backend.
...
└── LeadMapper / ChatwootConversationMapper
...
- **Application Service:** `SubmitLeadUseCase` orquesta guardar lead y crear conversation.
...
| **Integración Chatwoot** | Crear contacts/conversations vía Application API. | Sistema externo; lenguaje: contacto, conversación, inbox. |
...
| **De negocio: lead sin conversación** | Perdemos atención inmediata si Chatwoot falla. | Registrar `chatwoot_synced` y alertar si falla. |
...
| **Conversation** | Hilo de mensajes creado en Chatwoot a partir de un lead. |
...
| **Use Case** | Orquestación de un flujo de negocio, ej. guardar lead + crear conversation. |
